...
using tracedrive_one.Configuration.Helper;
using tracedrive_one.Configuration.StreamProvider;

#pragma warning disable 162

namespace tracedrive_one.Configuration.Store.Json
{
    /// <summary>
    /// JSON 配置文件转换流
    /// </summary>
    public class JsonConfigStore : IConfigStore
    {
        ...
        public event AsyncEventHandler SourceChanged
        {
            add
            {
                var streamProviderWithNotification = _configStream as IStreamProviderWithNotification;
                if (streamProviderWithNotification != null)
        ...
        private static ConfigData ExtraSettingFromString(string settingString)
        {
            ...
            while (stack.Any())
            {
                ...
                foreach (var item in curItem)
                {
                    ...
                    // 处理值
                    var jValue = item.Value as JValue;
                    if (jValue != null)
                    {
